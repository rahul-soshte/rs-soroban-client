...
pub fn assemble_transaction(
    ...
    let min_resource_fee_num = match success {
        SimulateTransactionResponse::Success(ref x) => x.min_resource_fee.parse::<u32>().unwrap_or(0),
        _ => panic!("Invalid"),
    };
    ...
    .fee(classic_fee_num + min_resource_fee_num).clone();
    // .build();
...
pub fn parse_raw_simulation(sim: Either<SimulateTransactionResponse, RawSimulateTransactionResponse>) -> SimulateTransactionResponse {
    ...
    let base = BaseSimulateTransactionResponse {
        ...
        events: match &sim {
            Either::Right(raw) => {
                raw.events.as_ref().unwrap_or(&vec![])
                    .iter()
...
fn parse_successful(sim: RawSimulateTransactionResponse, partial: BaseSimulateTransactionResponse) -> SimulateTransactionResponse {
    let success_data = {
        let mut base = SimulateTransactionSuccessResponse {
            transaction_data: SorobanDataBuilder::new(Some(soroban_data_builder::Either::Left(sim.transaction_data.unwrap()))), 
        ...
        if let Some(results) = &sim.results {
            if !results.is_empty() {
                base.result = Some(SimulateHostFunctionResult {
                    auth: results[0].auth.as_ref().unwrap_or(&vec![]).iter()
...
pub fn is_simulation_raw(sim: Either<SimulateTransactionResponse, RawSimulateTransactionResponse>) -> bool {
    match sim {
